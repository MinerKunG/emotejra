...
public class ModernChooseWheel implements IChooseWheel {
    ...
    public void render(GuiGraphics matrices, int mouseX, int mouseY, float delta){
        ...
        // --- แก้ไขส่วนแสดงเลขหน้า: ดึงค่าหน้าสูงสุดจาก Config ---
        ...
        // ปรับตำแหน่งตัวอักษรให้อยู่ตรงกลางตามความยาวของข้อความ
            ...
            if(element != null){
                ...
                if (selectedPageButton == 0) {
                    if (fastMenuPage > 0) {
                        ...
                        fastMenuPage = maxPages - 1; // วนกลับไปหน้าสุดท้าย
                    }
                } else if (selectedPageButton == 1) {
                    if (fastMenuPage < maxPages - 1) {
                        fastMenuPage += 1;
                    } else {
                        fastMenuPage = 0; // วนกลับไปหน้าแรก
                    }
                }
            }
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        if (amount < 0) { // เลื่อนลง (ถัดไป)
            fastMenuPage = (fastMenuPage + 1) % maxPages;
            return true;
        } else if (amount > 0) { // เลื่อนขึ้น (ย้อนกลับ)
            fastMenuPage = (fastMenuPage - 1 + maxPages) % maxPages;
            return true;
        }
        return false;
    }
